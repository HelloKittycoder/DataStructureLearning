package com.kittycoder.datastructure.search;

import java.util.Arrays;

/**
 * Created by shucheng on 2021/6/5 9:45
 * 斐波那契查找
 */
public class FinbonacciSearch {
    ...
    public static int fibSearch(int[] a, int key) {
        ...
        // 实际上就是使用a数组最后的数填充temp
        // 举例：
        //temp = {1,8, 10, 89, 1000, 1234, 0, 0}  => {1,8, 10, 89, 1000, 1234, 123, 1234}
        for (int i = high + 1; i < temp.length; i++) {
            temp[i] = a[high];
        }

        // 使用while来循环处理，找到我们的数key
        while (low <= high) { // 只要这个条件安祖，就可以找
            mid = low + f[k - 1] - 1;
